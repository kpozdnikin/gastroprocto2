...
angular.module('myApp.directives').directive('myDialogsListMobile', function ($window, $timeout) {
  return {
    link: link
  };
  ...
  function link($scope, element, attrs) {
    ...
    var panelWrapSelector = attrs.modal ? '.mobile_modal_body .im_dialogs_panel' : '.im_dialogs_panel';
    ...
    function updateSizes() {
      ...
      if (attrs.modal) {
        $(element).css({
          height: $($window).height() - (panelWrap ? panelWrap.offsetHeight : 58) - 46
        });
        return;
      }
      ...
      $(element).css({
        height: $($window).height() - (headWrap ? headWrap.offsetHeight : 46) - (panelWrap ? panelWrap.offsetHeight : 58) - parseInt($(dialogsColWrap).css('paddingBottom') || 0)
      });
    }
  ...
  function link($scope, element, attrs) {
    ...
    $(scrollableWrap).on('scroll', function (e) {
      if (!element.is(':visible') || $(scrollableWrap).hasClass('im_history_to_bottom')) {
        return;
      }
      var st = scrollableWrap.scrollTop;
  ...
  function link($scope, element, attrs) {
    ...
    function updateSizes() {
      $(element).css({
        height: $($window).height() - (panelWrap && panelWrap.offsetHeight || 0) - (searchWrap && searchWrap.offsetHeight || 0) - 64
      });
    }
  ...
  function link($scope, element, attrs) {
    ...
    function updateSizes() {
      $(element).css({
        height: $($window).height() - (panelWrap && panelWrap.offsetHeight || 0) - (searchWrap && searchWrap.offsetHeight || 0) - (46 + 18)
      });
    }
...
//# sourceMappingURL=directives_mobile-compiled.js.map