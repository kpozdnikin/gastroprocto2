...
EmojiTooltip.prototype.createTooltip = function () {
  ...
  var html = '<div class="composer_emoji_tooltip noselect">\
  ...
  angular.forEach(['emoji', 'stickers'], function (tabName, tabIndex) {
    var tab = $('.composer_emoji_tooltip_tab_' + tabName, self.tabsEl).on('mousedown', function (e) {
...
EmojiTooltip.prototype.updateStickersContents = function (force) {
  ...
  var scrollStickers = function scrollStickers() {
  ...
  var renderStickers = function renderStickers(stickersets) {
    var set;
    var docID;
    var i;
    var j;
    ...
    for (i = 0, len1 = stickersets.length; i < len1; i++) {
      ...
      if (set.title) {
        html.push('<a class="composer_stickerset_title', set.id ? '' : ' disabled', '" data-stickerset="', encodeEntities(set.short_name), '">', encodeEntities(set.title), '</a>');
      }
      if (!set.id) {
...
EmojiTooltip.prototype.onStickersScroll = function (scrollable, scrollTop) {
  ...
  if (scrollTop < 20) {
    ...
    for (i = 0; i < len; i++) {
      ...
      if (scrollTop >= currentPos[0] && scrollTop < currentPos[0] + currentPos[1]) {
        currentCat = i;
        break;
      }
    }
...
MessageComposer.prototype.checkAutocomplete = function (forceFull) {
  ...
  if (value && this.curInlineResults && this.curInlineResults.text == value) {
...
MessageComposer.prototype.setFocusedValue = function (parts) {
  ...
  if (this.richTextareaEl) {
    this.selId = (this.selId || 0) + 1;
    var html = this.getRichHtml(prefix) + '<span id="composer_sel' + this.selId + '">' + this.getRichHtml(selection) + '</span>' + this.getRichHtml(suffix);
...
MessageComposer.prototype.getRichHtml = function (text) {
  ...
  html = html.replace(/  /g, ' \xA0').replace(/^ | $/g, '\xA0');
...
MessageComposer.prototype.showInlineSuggestions = function (botResults) {
  ...
  var self = this;
  if (self.autoCompleteScope.type == 'inline' && self.autoCompleteScope.botResults == botResults && self.autocompleteShown) {
    return;
  }
  setZeroTimeout(function () {
    self.autoCompleteScope.$apply(function () {
...
Scroller.prototype.scrollToNode = function (node) {
  ...
  if (scrollTop > elTop) {
    // we are below the node to scroll
    this.scrollTo(elTop);
  } else if (scrollTop < elTop + elHeight - viewportHeight) {
    // we are over the node to scroll
    this.scrollTo(elTop + elHeight - viewportHeight);
  }
};

//# sourceMappingURL=message_composer-compiled.js.map