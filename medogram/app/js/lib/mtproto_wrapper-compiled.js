...
angular.module('izhukov.mtproto.wrapper', ['izhukov.utils', 'izhukov.mtproto']).factory('MtpApiManager', function ($cookies, Storage, MtpAuthorizer, MtpNetworkerFactory, MtpSingleInstanceService, AppRuntimeManager, ErrorService, qSync, $rootScope, $q, TelegramMeWebService) {
  ...
  function mtpGetNetworker(dcID, options) {
    ...
    var cache = options.fileUpload || options.fileDownload ? cachedUploadNetworkers : cachedNetworkers;
    if (!dcID) {
  ...
  function mtpInvokeApi(method, params, options) {
          ...
          if (!error.handled) {
            if (error.code == 401) {
              mtpLogOut()['finally'](function () {
                if (location.protocol == 'http:' && !Config.Modes.http && Config.App.domains.indexOf(location.hostname) != -1) {
    ...
    var performRequest = function performRequest(networker) {
      return (cachedNetworker = networker).wrapApiCall(method, params, options).then(function (result) {
  ...
  function downloadSmallFile(location) {
    ...
    return cachedDownloadPromises[fileName] = fileStorage.getFile(fileName).then(function (blob) {
      ...
      var processDownloaded = function processDownloaded(bytes) {
        if (!location.sticker || WebpManager.isWebpSupported()) {
  ...
  function downloadFile(dcID, location, size, options) {
    ...
    fileStorage.getFile(fileName, size).then(function (blob) {
      ...
      fileWriterPromise.then(function (fileWriter) {
        ...
        for (offset = startOffset; offset < size; offset += limit) {
          writeFileDeferred = $q.defer();(function (isFinal, offset, writeFileDeferred, writeFilePromise) {
    ...
    deferred.promise.cancel = function () {
      if (!canceled && !resolved) {
        ...
        _errorHandler({ type: 'DOWNLOAD_CANCELED' });
      }
    };
  ...
  function uploadFile(file) {
    ...
    var deferred = $q.defer();
    var _errorHandler2 = function errorHandler(error) {
      // console.error('Up Error', error)
      deferred.reject(error);
      canceled = true;
      _errorHandler2 = angular.noop;
    },
        part = 0,
        offset,
        resultInputFile = {
    ...
    deferred.promise.cancel = function () {
      ...
      if (!canceled && !resolved) {
        canceled = true;
        _errorHandler2({ type: 'UPLOAD_CANCELED' });
      }
    };
...
//# sourceMappingURL=mtproto_wrapper-compiled.js.map