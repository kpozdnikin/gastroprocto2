...
function hasOnlick(element) {
  if (element.onclick || element.getAttribute('ng-click')) {
    return true;
  }
...
function setRichFocus(field, selectNode, noCollapse) {
  field.focus();
  if (selectNode && selectNode.parentNode == field && !selectNode.nextSibling && !noCollapse) {
...
function getSelectedText() {
  var sel = (window.getSelection && window.getSelection() || document.getSelection && document.getSelection() || document.selection && document.selection.createRange().text || '').toString().replace(/^\s+|\s+$/g, '');
...
if (Config.Modes.animations && typeof window.requestAnimationFrame == 'function') {
...
function versionCompare(ver1, ver2) {
  ...
  var a = Math.max(ver1.length, ver2.length),
      i;
