...
angular.module('izhukov.mtproto', ['izhukov.utils']).factory('MtpDcConfigurator', function () {
  ...
  var dcOptions = Config.Modes.test ? [{ id: 1, host: '149.154.175.10', port: 80 }, { id: 2, host: '149.154.167.40', port: 80 }, { id: 3, host: '149.154.175.117', port: 80 }] : [{ id: 1, host: '149.154.175.50', port: 80 }, { id: 2, host: '149.154.167.51', port: 80 }, { id: 3, host: '149.154.175.100', port: 80 }, { id: 4, host: '149.154.167.91', port: 80 }, { id: 5, host: '149.154.171.5', port: 80 }];
  ...
  */
  ...
  MtpNetworker.prototype.checkLongPoll = function (force) {
    ...
    Storage.get('dc').then(function (baseDcID) {
      if (isClean && (baseDcID != self.dcID || self.upload || self.sleepAfter && tsNow() > self.sleepAfter)) {
  ...
  MtpNetworker.prototype.getMsgKeyIv = function (msgKey, isOut) {
    ...
    return $q.all(promises).then(function (result) {
      ...
      sha1a = new Uint8Array(result.sha1a), sha1b = new Uint8Array(result.sha1b), sha1c = new Uint8Array(result.sha1c), sha1d = new Uint8Array(result.sha1d);
    ...
    angular.forEach(this.pendingMessages, function (value, messageID) {
      if (!value || value >= currentTime) {
        if (message = self.sentMessages[messageID]) {
          ...
          if (!message.notContentRelated && messagesByteLen && messagesByteLen + messageByteLength > 655360) {
            // 640 Kb
            lengthOverflow = true;
            return;
          }
    ...
    return this.getEncryptedMessage(data.getBuffer()).then(function (encryptedResult) {
      ...
      return requestPromise.then(function (result) {
        ...
        if (error.status == 404 && (error.data || '').indexOf('nginx/0.3.33') != -1) {
          Storage.remove('dc' + self.dcID + '_server_salt', 'dc' + self.dcID + '_auth_key').then(function () {
    ...
    return this.getDecryptedMessage(msgKey, encryptedData).then(function (dataWithPadding) {
      ...
      return CryptoWorker.sha1Hash(hashData).then(function (dataHash) {
        ...
        var deserializerOptions = {
          mtproto: true,
          override: {
            mt_message: function mt_message(result, field) {
            ...
            mt_rpc_result: function mt_rpc_result(result, field) {
    ...
    switch (message._) {
      ...
      case 'bad_msg_notification':
        ...
        if (message.error_code == 16 || message.error_code == 17) {
          if (MtpTimeManager.applyServerTime(bigStringInt(messageID).shiftRight(32).toString(10))) {
  ...
  return {
    getNetworker: function getNetworker(dcID, authKey, serverSalt, options) {
      return new MtpNetworker(dcID, authKey, serverSalt, options);
    },
    setUpdatesProcessor: function setUpdatesProcessor(callback) {
...
//# sourceMappingURL=mtproto-compiled.js.map