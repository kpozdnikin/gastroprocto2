...
angular.module('myApp.controllers', ['myApp.i18n'])
  ...
  .controller('AppLoginController', function ($http, $scope, $rootScope, $location, $timeout, $modal, $modalStack, AppUsersManager, MtpApiManager, ErrorService, NotificationsManager, PasswordManager, ChangelogNotifyService, IdleManager, LayoutSwitchService, TelegramMeWebService, _) {
  ...
  .controller('AppIMController', function ($q, $http, qSync, $scope, $location, $routeParams, $modal, $rootScope, $modalStack, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ContactsSelectService, ChangelogNotifyService, ErrorService, AppRuntimeManager, HttpsMigrateService, LayoutSwitchService, LocationParamsService, AppStickersManager) {
    ...
    $scope.importMedogramContact = function(){

      //userID
      $scope.importedContact = {
        phone: '79114226128',
        first_name: 'Ольга',
        last_name: 'Поздникина'
      }

      $scope.progress = {enabled: true}
      AppUsersManager.importContact(
          $scope.importedContact.phone,
          $scope.importedContact.first_name || '',
          $scope.importContact.last_name || ''
      ).then(function (foundUserID) {
        console.log('contact importing');
        if (!foundUserID) {
          console.log('contact was not imported');
          ErrorService.show({
            error: {code: 404, type: 'USER_NOT_USING_TELEGRAM'}
          })
        }
        else{
          console.log('contact was imported ' + foundUserID);
          $scope.dialogSelect('u278411087_5535194232277399468');
          var dateNow = new Date();
          //Запоминаем так же текущего юзера
          $scope.userData = AppUsersManager.getSelf();
          var data = {
            _id: '',
            firstName: $scope.userData.first_name,
            lastName: $scope.userData.last_name,
            telegram: String($scope.userData.id),
