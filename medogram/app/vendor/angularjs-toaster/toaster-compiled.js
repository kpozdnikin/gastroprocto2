...
(function (window, document) {
    ...
    angular.module('toaster', []).constant('toasterConfig', {
        ...
        function createTypeMethod(toasterType) {
            return function (title, body, timeout, bodyOutputType, clickHandler, toasterId, showCloseButton, toastId, onHideCallback) {
                if (angular.isString(title)) {
                    return this.pop(toasterType, title, body, timeout, bodyOutputType, clickHandler, toasterId, showCloseButton, toastId, onHideCallback);
                } else {
                    // 'title' is actually an object with options
        ...
        toasterFactory = {
            ...
            subscribeToNewToastEvent: function subscribeToNewToastEvent(onNewToast) {
                newToastEventSubscribers.push(onNewToast);
            },
            subscribeToClearToastsEvent: function subscribeToClearToastsEvent(onClearToasts) {
                clearToastsEventSubscribers.push(onClearToasts);
            },
            unsubscribeToNewToastEvent: function unsubscribeToNewToastEvent(onNewToast) {
            ...
            unsubscribeToClearToastsEvent: function unsubscribeToClearToastsEvent(onClearToasts) {
            ...
            link: function link(scope, elm, attrs) {
                scope.$watch('directiveName', function (directiveName) {
                    ...
                    try {
                        ...
                        throw new Error(directiveName + ' could not be found. ' + 'The name should appear as it exists in the markup, not camelCased as it would appear in the directive declaration,' + ' e.g. directive-name not directiveName.');
                    }

                    var directiveDetails = directive[0];

                    if (directiveDetails.scope !== true && directiveDetails.scope) {
                        throw new Error('Cannot use a directive with an isolated scope. ' + 'The scope must be either true or falsy (e.g. false/null/undefined). ' + 'Occurred for directive ' + directiveName + '.');
                    }

                    if (directiveDetails.restrict.indexOf('A') < 0) {
                        throw new Error('Directives must be usable as attributes. ' + 'Add "A" to the restrict option (or remove the option entirely). Occurred for directive ' + directiveName + '.');
                    }
        ...
        return {
            ...
            link: function link(scope, elm, attrs) {
                ...
                scope.configureTimer = function (toast) {
                    ...
                    if ((typeof timeout === 'undefined' ? 'undefined' : _typeof(timeout)) === "object") timeout = timeout[toast.type];
                    if (timeout > 0) {
                ...
                function addToast(toast, toastId) {
                    ...
                    } else if ((typeof closeButton === 'undefined' ? 'undefined' : _typeof(closeButton)) === "object") {
