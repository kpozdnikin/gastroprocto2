...
angular.module('mediaPlayer', ['mediaPlayer.helpers']).constant('mp.playerDefaults', {
  ...
  formatTime: '00:00',
  loadPercent: 0
}).directive('mediaPlayer', ['$rootScope', '$interpolate', '$timeout', 'mp.throttle', 'mp.playerDefaults', function ($rootScope, $interpolate, $timeout, throttle, playerDefaults) {
  ...
  var playerMethods = {
     ...
     */
    load: function load(mediaElement, autoplay) {
      // method overloading
      if (typeof mediaElement === 'boolean') {
        autoplay = mediaElement;
        mediaElement = null;
      } else if ((typeof mediaElement === 'undefined' ? 'undefined' : _typeof(mediaElement)) === 'object') {
     ...
     */
    play: function play(index, selectivePlay) {
      // method overloading
      ...
      if (!this.currentTrack && this.$domEl.readyState) {
        this.currentTrack++;
      }
    ...
    playPause: function playPause(index, selectivePlay) {
      // method overloading
      if (typeof index === 'boolean') {
        selectivePlay = index;
        index = undefined;
    ...
    seek: function seek(value) {
      ...
      if (typeof value === 'string') {
        ...
        if (valuesArr.length) {
          doubleval += parseInt(valuesArr.pop(), 10) * 60;
        }
        if (valuesArr.length) {
          doubleval += parseInt(valuesArr.pop(), 10) * 3600;
        }
        if (!isNaN(doubleval)) {
    ...
    $addSourceList: function $addSourceList(sourceList) {
      var self = this;
      if (angular.isArray(sourceList)) {
        angular.forEach(sourceList, function (singleElement, index) {
          ...
          ['src', 'type', 'media'].forEach(function (key) {
            // use only a subset of the properties
            if (singleElement[key] !== undefined) {
              // firefox is picky if you set undefined attributes
    ...
    $clearSourceList: function $clearSourceList() {
    ...
    $attachPlaylist: function $attachPlaylist(pl) {
   ...
   */
  var bindListeners = function bindListeners(au, al, element) {
    var listeners = {
      ...
      loadedmetadata: function loadedmetadata() {
        au.$apply(function (scope) {
          if (!scope.currentTrack) {
            scope.currentTrack++;
          } // This is triggered *ONLY* the first time a <source> gets loaded.
      ...
      volumechange: function volumechange() {
        // Sent when volume changes (both when the volume is set and when the muted attribute is changed).
        au.$apply(function (scope) {
  ...
  var MediaPlayer = function MediaPlayer(element) {
  ...
  function playlistWatch(player) {
    return function (playlistNew, playlistOld, watchScope) {
      ...
      if (player.currentTrack) {
        ...
        if (newTrackNum !== null) {
          // currentTrack it's still in the new playlist, update trackNumber
          player.currentTrack = newTrackNum + 1;
          player.tracks = playlistNew.length;
        } else {
          // currentTrack has been removed.
          player.pause();
          if (playlistNew.length) {
            // if the new playlist has some elements, replace actual.
            $timeout(function () {
              // need $timeout because the mediaTag needs a little time to launch the 'pause' event
            ...
            // the new playlist has no elements, clear actual
            player.reset();
          }
        }
      } else if (playlistNew.length) {
        // new playlist has elements, load them
        player.$clearSourceList();
        player.$addSourceList(playlistNew[0]);
        player.load();
        player.tracks = playlistNew.length;
      } else {
        // new playlist has no elements, clear actual
        player.reset();
      }
    };
  }
  ...
  return {
    ...
    link: function link(scope, element, attrs, ctrl) {
      ...
      var player = new MediaPlayer(element),
...
angular.module('mediaPlayer.helpers', []).factory('mp.throttle', ['$timeout', function ($timeout) {
...
//# sourceMappingURL=angular-media-player-compiled.js.map