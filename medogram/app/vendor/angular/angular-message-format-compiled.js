...
(function (window, angular) {
    ...
    parsedFn['$$watchDelegate'] = function watchDelegate(scope, listener, objectEquality) {
      var unwatch = scope['$watch'](noop, function textLiteralWatcher() {
        if (isFunction(listener)) {
          listener.call(null, text, text, scope);
        }
        unwatch();
      }, objectEquality);
      return unwatch;
  ...
  function subtractOffset(expressionFn, offset) {
    ...
    parsedFn['$$watchDelegate'] = function watchDelegate(scope, listener, objectEquality) {
      unwatch = scope['$watch'](expressionFn, function pluralExpressionWatchListener(newValue, oldValue) {
        if (isFunction(listener)) {
          listener.call(null, minusOffset(newValue), minusOffset(oldValue), scope);
        }
      }, objectEquality);
      return unwatch;
    };
  ...
  function MessageSelectorWatchers(msgSelector, scope, listener, objectEquality) {
    ...
    var expressionFnListener = function expressionFnListener(newValue, oldValue) {
  ...
  MessageSelectorWatchers.prototype.expressionFnListener = function expressionFnListener(newValue, oldValue) {
    ...
    var messageFnListener = function messageFnListener(newMessage, oldMessage) {
  ...
  InterpolationParts.prototype.toParsedFn = function toParsedFn(mustHaveExpression, originalText) {
    ...
    var parsedFn = function parsedFn(context) {
  ...
  MessageFormatParser.prototype.errorInParseLogic = function errorInParseLogic() {
    throw $interpolateMinErr('logicbug', 'The messageformat parser has encountered an internal error.  Please file a github issue against the AngularJS project and provide this message text that triggers the bug.  Text: “{0}”', this.text);
  };
  ...
  MessageFormatParser.prototype.errorExpecting = function errorExpecting() {
    ...
    if (match == null) {
      ...
      throw $interpolateMinErr('reqarg', 'Expected one of “plural” or “select” at line {0}, column {1} of text “{2}”', position.line, position.column, this.text);
    }
    var word = match[1];
    if (word == "select" || word == "plural") {
      position = indexToLineAndColumn(this.text, this.index);
      throw $interpolateMinErr('reqcomma', 'Expected a comma after the keyword “{0}” at line {1}, column {2} of text “{3}”', word, position.line, position.column, this.text);
    } else {
      position = indexToLineAndColumn(this.text, this.index);
      throw $interpolateMinErr('unknarg', 'Unsupported keyword “{0}” at line {0}, column {1}. Only “plural” and “select” are currently supported.  Text: “{3}”', word, position.line, position.column, this.text);
    }
  };
  ...
  MessageFormatParser.prototype.ruleString = function ruleString() {
    ...
    if (match == null) {
      ...
      throw $interpolateMinErr('wantstring', 'Expected the beginning of a string at line {0}, column {1} in text “{2}”', position.line, position.column, this.text);
    }
  ...
  MessageFormatParser.prototype.ruleInsideString = function ruleInsideString() {
    ...
    if (match == null) {
      ...
      throw $interpolateMinErr('untermstr', 'The string beginning at line {0}, column {1} is unterminated in text “{2}”', position.line, position.column, this.text);
    }
    var chars = match[0];
  ...
  MessageFormatParser.prototype.rulePluralOrSelect = function rulePluralOrSelect() {
    ...
    switch (argType) {
      case "plural":
        this.rule = this.rulePluralStyle;break;
      case "select":
        this.rule = this.ruleSelectStyle;break;
      default:
        this.errorInParseLogic();
    }
  };
  ...
  MessageFormatParser.prototype.assertChoiceKeyIsNew = function assertChoiceKeyIsNew(choiceKey, index) {
    if (this.choices[choiceKey] !== void 0) {
      ...
      throw $interpolateMinErr('dupvalue', 'The choice “{0}” is specified more than once. Duplicate key is at line {1}, column {2} in text “{3}”', choiceKey, position.line, position.column, this.text);
    }
  };
  ...
  MessageFormatParser.prototype.ruleMessageText = function ruleMessageText() {
    if (!this.consumeRe(BRACE_OPEN_RE)) {
      var position = indexToLineAndColumn(this.text, this.index);
      throw $interpolateMinErr('reqopenbrace', 'The plural choice “{0}” must be followed by a message in braces at line {1}, column {2} in text “{3}”', this.choiceKey, position.line, position.column, this.text);
    }
  ...
  MessageFormatParser.prototype.advanceInInterpolationOrMessageText = function advanceInInterpolationOrMessageText() {
    ...
    if (this.ruleChoiceKeyword == null) {
      ...
      if (match == null) {
        ...
        throw $interpolateMinErr('reqendbrace', 'The plural/select choice “{0}” message starting at line {1}, column {2} does not have an ending closing brace. Text “{3}”', this.choiceKey, position.line, position.column, this.text);
      }
    }
    // match is non-null.
    var token = match[0];
  ...
  MessageFormatParser.prototype.ruleEndMustache = function ruleEndMustache() {
    ...
    if (match == null) {
      var position = indexToLineAndColumn(this.text, this.index);
      throw $interpolateMinErr('reqendinterp', 'Expecting end of interpolation symbol, “{0}”, at line {1}, column {2} in text “{3}”', '}}', position.line, position.column, this.text);
    }
  ...
  MessageFormatParser.prototype.ruleInAngularExpression = function ruleInAngularExpression() {
    ...
    if (match == null) {
      ...
      throw $interpolateMinErr('badexpr', 'Unexpected end of Angular expression.  Expecting operator “{0}” at the end of the text “{1}”', this.getEndOperator(innermostOperator), this.text);
    }
    ...
    if (operator == ",") {
      if (this.trustedContext) {
        position = indexToLineAndColumn(this.text, this.index);
        throw $interpolateMinErr('unsafe', 'Use of select/plural MessageFormat syntax is currently disallowed in a secure context ({0}).  At line {1}, column {2} of text “{3}”', this.trustedContext, position.line, position.column, this.text);
      }
    ...
    if (this.angularOperatorStack.length > 0) {
      ...
      position = indexToLineAndColumn(this.text, this.index);
      throw $interpolateMinErr('badexpr', 'Unexpected operator “{0}” at line {1}, column {2} in text. Was expecting “{3}”. Text: “{4}”', operator, position.line, position.column, getEndOperator(this.angularOperatorStack[0]), this.text);
    }
...
//# sourceMappingURL=angular-message-format-compiled.js.map